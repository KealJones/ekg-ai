...
import {
  ...
  ekgBenchBabi20,evaluateDevelopmentalProbes
} from '../dist/index.js';
...
function school(graph){return new TeacherSchool(graph,ekgCapabilities(),new MemoryProgramLibrary())}
...
test('bAbI #4/#5 are red before relation-role education',async()=>{
  ...
  const r=await evaluateDevelopmentalProbes(agent(graph),ekgBenchBabi20.slice(3,5));
...
test('FrameNet-style learned event roles turn bAbI #5 green',async()=>{
  ...
  const [r]=await evaluateDevelopmentalProbes(agent(graph),[ekgBenchBabi20[4]]);
