...
import {
  T,MemoryGraphStore,MemoryProgramLibrary,MemoryEpisodeStore,CapabilityRegistry,babyCapabilities,
...
test('validated Teacher program becomes durable capability identity, not a one-off answer',()=>{
  const graph=new MemoryGraphStore(), programs=new MemoryProgramLibrary(), caps=babyCapabilities();
...
test('future synthesis can call an acquired learned capability as part of a new program',()=>{
  const full=babyCapabilities(), caps=new CapabilityRegistry();
...
test('Teacher impasse now includes actual capability catalog instead of inviting invented tools',()=>{
  const graph=new MemoryGraphStore(), caps=babyCapabilities();
